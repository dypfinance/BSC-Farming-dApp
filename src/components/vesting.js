...
export default function initVesting({ staking, buyers, apr, liquidity='ETH', lock, expiration_time }) {
    ...
    class Vesting extends React.Component {
        ...
        refreshBalance = async () => {
            ...
            if(stakingAddress == '0x055cd97920d9380facdc833991cc93b3be177974'){
                                        ...
                                        <td className="text-right"><strong>{pendingDivs}</strong> <small>DYP</small></td>
                                                ...
                                                <tr>
                                                    <td style={{ fontSize: '1rem', paddingTop: '2rem' }} colSpan='2' className='text-center'>
                                                        <a target='_blank' rel='noopener noreferrer' href={`${window.config.etherscan_baseURL}/token/${reward_token_idyp._address}?a=${coinbase}`}>View Transaction History on BscScan</a> &nbsp; <i style={{ fontSize: '.8rem' }} className='fas fa-external-link-alt'></i>
                                                    </td>
                                                </tr>
                                                {/*<tr>*/}
