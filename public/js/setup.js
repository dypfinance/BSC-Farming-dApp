...
const TOKENS_DISBURSED_PER_YEAR = [
  ...
  660_000, 996_000, 1_680_000, 2_220_000, 2_760_000,
];
...
window.config = {
  ...
  constant_stakingvesting_30_address:
    "0x4d9299545020aBDA0bbeaF8561a6E90804cB96Cf",
  //vesting-staking
...
Object.keys(window.config)
  .filter(
    (k) =>
      ...
      (k.startsWith("farming_new_") && k.endsWith("_address"))
  )
  .forEach((k) => {
...
async function getContract(key) {
  ...
  if (!window.cached_contracts_connected[key]) {
    window.cached_contracts_connected[key] = new window.bscweb3.eth.Contract(
      ABI,
      address,
  ...
  if (!window.IS_CONNECTED) return window.cached_contracts_connected[key];
...
// constant staking
window.constant_stakingvesting_30 = new CONSTANT_STAKING(
  "CONSTANT_STAKINGVESTING_30"
);
...
//Constant staking DYP -> DYP 30%
window.constant_staking_new10 = new CONSTANT_STAKING_NEW(
  "CONSTANT_STAKINGIDYP_3"
);
window.constant_staking_new11 = new CONSTANT_STAKING_NEW(
  "CONSTANT_STAKINGIDYP_4"
);
...
/* Constant Staking iDYP */
window.constant_staking_idyp_1 = new CONSTANT_STAKING_NEW(
  "CONSTANT_STAKINGIDYP_1"
);
window.constant_staking_idyp_2 = new CONSTANT_STAKING_NEW(
  "CONSTANT_STAKINGIDYP_2"
);
window.constant_staking_idyp_5 = new CONSTANT_STAKING_NEW(
  "CONSTANT_STAKINGIDYP_5"
);
window.constant_staking_idyp_6 = new CONSTANT_STAKING_NEW(
...
function get_usd_values({ token_contract_addresses, lp_ids }) {
  return new Promise(async (resolve, reject) => {
    ...
    let usdPerPlatformToken = await getPrice(
    ...
    async function getData(token_contract_addresses, lp_ids) {
      ...
      for (let id of token_contract_addresses) {
        ...
        else token_price_usd = parseFloat(_amountOutMin);
      ...
      for (let lp_id of lp_ids) {
        ...
        if (pairAddress == "0x1bc61d08a300892e784ed37b2d0e63c85d1d57fb") {
          platformTokenContract = new window.bscweb3.eth.Contract(
            window.TOKEN_ABI,
            window.config.reward_token_address2,
            { from: await getCoinbase() }
          );
...
async function getTokenHolderBalance(token, holder) {
  let tokenContract = new window.bscweb3.eth.Contract(window.TOKEN_ABI, token, {
    from: await getCoinbase(),
  });
  return await tokenContract.methods.balanceOf(holder).call();
}
...
async function approveToken(token, spender, amount) {
  let tokenContract = new window.web3.eth.Contract(window.TOKEN_ABI, token, {
    from: await getCoinbase(),
  });
...
async function getUniswapRouterContract(
  ...
  return new window.bscweb3.eth.Contract(window.UNISWAP_ROUTER_ABI, address, {
    from: await getCoinbase(),
  });
}
